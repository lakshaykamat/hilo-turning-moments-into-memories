...
ChatRoomSchema.statics.findByUser = function(userId) {
  return this.find({ users: userId }).populate('users', 'username').populate('createdBy', 'username');
};

// Static method to find chat rooms by type
ChatRoomSchema.statics.findByType = function(type) {
  return this.find({ type }).populate('users', 'username').populate('createdBy', 'username');
};
...
ChatRoomSchema.statics.removeUserFromChatRoom = async function(chatRoomId, userId) {
  const chatRoom = await this.findById(chatRoomId);
  if (chatRoom) {
    chatRoom.users = chatRoom.users.filter(user => user.toString() !== userId);
    await chatRoom.save();
  }
  return chatRoom;
};
